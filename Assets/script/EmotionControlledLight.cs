...
public class EmotionControlledLight : MonoBehaviour
{
    ...
    private IEnumerator ControlLightRoutine()
    {
        while (true)
        {
            ...
            for (int i = 0; i < emotionList.Count; i++)
            {
                ...
                {
                    time += Time.deltaTime;
                    float rotationX = Mathf.Lerp(-90f, -110f, confidence); // 置信度相关X轴旋转
                    float rotationZ = Mathf.Lerp(30f, -30f, Mathf.PingPong(time * rotationSpeedMultiplier * confidence, 1)); // Z轴循环旋转
